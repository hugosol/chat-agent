...
| 原则 | 说明 |
|------|------|
| **非 SPA 多页面** | 保留 Spring Boot 管理的多 HTML 页面，不引入路由库（React Router）。每页通过 Vite Library Mode 构建为独立 IIFE bundle。现有页面：Chat（`index.html`）、Manage（`manage/index.html`）、Review（`review/index.html`）、Movies（`movies/index.html`）、Tune（`tune/index.html`）、Settings（`settings/index.html`）、Profile（`profile/index.html`）、Login（`login/index.html`） |
...
│   ├── movies/         # Movies 页面组件
│   │   ├── MoviesApp.tsx       # 页面容器（列表 + 分页状态管理）
│   │   ├── MovieToolbar.tsx     # 搜索/排序/添加/导入按钮
│   │   ├── MovieList.tsx        # 电影分页列表
│   │   ├── MovieBlock.tsx       # 单部电影行（标题 + 字幕状态 + 操作）
│   │   ├── MovieSearchModal.tsx  # TMDB 搜索模态框
│   │   ├── MovieImportModal.tsx  # CSV 批量导入模态框
│   │   ├── MovieDeleteModal.tsx  # 删除确认模态框
│   │   └── MovieRetryModal.tsx   # 字幕重试确认模态框
│   ├── review/         # Review 页面组件
│   │   └── CardDisplay.tsx      # 卡面展示（翻转 + 增强区域 + 内联编辑）
│   └── manage/         # Manage 页面组件
...
### 5.3 Settings / Tune 页面：本地 useState

Settings 和 Tune 页面独立于其他页面，使用本地 useState 管理状态。Settings 通过 GET/PUT `/api/user/preferences` 读写 9 个表单字段。Tune 通过 GET `/api/tune/*` 查询 ReviewLog 总数和优化日志列表，admin 可切换查看其他用户数据。
...
### 8.5 测试数量（当前：294 tests, 36 files）
...
| 日期 | 内容 |
|------|------|
| 2026-06-07 | 新增 Tune 页面（`/tune`）：ReviewLog 总数 + Optimize/Reschedule 日志列表，admin 用户选择器。Review CardDisplay 新增翻转后内联编辑 back 功能。更新页面列表、状态管理、测试数量 |
