...
| 原则 | 说明 |
|------|------|
| **非 SPA 多页面** | 保留 Spring Boot 管理的多 HTML 页面，不引入路由库（React Router）。每页通过 Vite Library Mode 构建为独立 IIFE bundle |
...
```
ChatProvider (Context + useReducer + WS 生命周期)
  ├─ state: ChatState（appStatus, messages[], corrections[], tokenUsage, ...）
  ├─ dispatch: (action: Action) => void
  └─ send: (msg: ClientMessage) => void

消费组件通过 useChatContext() 获取 { state, dispatch, send }
```

| 组件 | 消费的 Context 字段 |
|------|-------------------|
| Header | `state.tokenUsage` |
| CorrectionSidebar | `state.corrections` |
| MessageList | `state.messages`, `state.corrections`, `state.streamInProgress` |
| ChatInput | `state.streamInProgress`, `state.appStatus`, `send` |
| Footer | `state.appStatus`, `send` |
| StatusBar | `state.appStatus`, `state.statusPayload` |
| ReportModal | `state.report` |

**WebSocket 生命周期：** 完全在 `ChatProvider` 的 `useEffect` 中管理——connect → onopen（auto-resume from localStorage）→ onmessage（dispatch Action）→ onclose / onerror。

**Action 类型（11 种）：** `SESSION_STARTED`, `AGENT_STREAM_DELTA`, `AGENT_STREAM_END`, `CORRECTION_RESULT`, `STATE_UPDATE`, `SESSION_RESUMED`, `WS_CLOSED`, `USER_MESSAGE_SENT`, `SESSION_REPORT`, `SET_APP_STATUS`, `DISMISS_REPORT`

**没有 vanilla bridge：** ChatProvider 直接处理所有 WebSocket 消息类型（包括 `TOKEN_WARNING`, `ERROR`, `SESSION_REPORT`），通过 `dispatch(action)` 进入 reducer。不需要双路分发。
