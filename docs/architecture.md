...
| 48 | React 渐进迁移 | 引入 Vite + React 18 + TypeScript 作为前端构建工具链。首阶段：`shared/nav.js` → `Header.tsx`（产物 `header-bundle.js`/`header-bundle.css`）。第二阶段：CorrectionSidebar 迁入 React（产物 `correction-sidebar-bundle.js`/`correction-sidebar-bundle.css`），建立多入口构建模式（每个入口独立 IIFE bundle + CSS 文件），纯组件 + 连接器模式（纯展示组件通过命令式 API 桥接 vanilla JS）。后续阶段：MessageList、ReportModal、DebugPanel、InputBar 全部迁入 React + `useReducer + context` 集中状态管理。React 本地托管在 `static/shared/`，CSS Modules 隔离样式，Vitest 做组件测试。Vite Library Mode 输出 IIFE bundle。构建挂接到 Maven `exec-maven-plugin` 的 `process-resources` 阶段；Vitest 挂在 `process-test-classes`。多入口构建：两个独立 Vite 配置（`vite.config.ts` + `vite.config.correction-sidebar.ts`），`package.json` build 脚本串联执行。共享代码采用内联策略——各页面级组件 bundle 直接 import 共享源码，Vite/Rollup 自动内联打包。不引入路由/状态管理库，不做 SPA。详见 ADR `frontend-react-migration.md`。 |
| 49 | Chat 页面 React 集中状态管理 | 三期路线图：Phase 1（CorrectionSidebar 独立模块，命令式 API 桥接）→ Phase 2（抽取 WebSocket 服务层 + `useReducer + context` 集中状态管理）→ Phase 3（剩余模块全部迁入 React，统一 React 树）。Phase 1 通过纯组件 + 连接器模式前向兼容：纯展示组件（props in, callbacks out）由 entry wrapper 通过本地 `useState` 管理状态，对外暴露命令式 API。该设计与 Phase 2 的 `useReducer + context` 直接映射（组件 props → reducer state，命令式方法 → action type）。选择 `useReducer + context` 而非 Zustand/Redux 等外部库：无额外依赖、与 React 18 原生集成、支持渐进采用。详见 ADR `centralized-chat-state.md`。 |
