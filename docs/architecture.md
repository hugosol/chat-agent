...
| 48 | React 渐进迁移 | 引入 Vite + React 18 + TypeScript 作为前端构建工具链。Phase 1：Header.tsx（产物 `header-bundle.js`/`header-bundle.css`）+ CorrectionSidebar 迁入 React（产物 `correction-sidebar-bundle.js`/`correction-sidebar-bundle.css`），建立多入口构建模式。Phase 2：抽取 WebSocket 服务层 + `useReducer + context` 集中状态管理（`chat-bundle.js`/`chat-bundle.css`，含 ChatProvider + Header + CorrectionSidebar + WS Hook）。React 本地托管在 `static/shared/`，CSS Modules 隔离样式，Vitest 做组件测试。Vite Library Mode 输出 IIFE bundle。构建挂接到 Maven `exec-maven-plugin` 的 `process-resources` 阶段；Vitest 挂在 `process-test-classes`。多入口构建：三个独立 Vite 配置（`vite.config.ts` + `vite.config.correction-sidebar.ts` + `vite.config.chat.ts`），`package.json` build 脚本串联执行。共享代码采用内联策略。Phase 3：MessageList、ReportModal、DebugPanel、InputBar 全部迁入 React。不引入路由/状态管理库，不做 SPA。详见 ADR `frontend-react-migration.md`。 |
| 49 | Chat 页面 React 集中状态管理 | 三期路线图：Phase 1（CorrectionSidebar 独立模块，命令式 API 桥接）→ **Phase 2 已完成**（抽取 WebSocket 服务层 + `useReducer + context` 集中状态管理）→ Phase 3（剩余模块全部迁入 React，统一 React 树）。Phase 2 成果：`chatReducer` 纯函数（7 种 WS 消息 + `WS_CLOSED`，8 个 Vitest 测试），`ChatProvider + useChatContext`（React 原生 context + `useReducer`，Header 和 CorrectionSidebar 通过 React portals 接入），`useChatWebSocket` Hook（WS 连接生命周期 + 双重路径：dispatch→reducer + vanilla callbacks→app.js），`ChatAgent.registerHandler()`/`ChatAgent.send()` 替代 app.js 的 `connect()`/`ws`。TOKEN_WARNING 和 LLM ERROR 改为 Toast 通知。`window.updateTokenBar()` 和 `window.__sidebarApi` 已退役。4 处 Phase 2 胶水代码标注 `// Phase 2 compat — Phase 3 移除`。详见 ADR `centralized-chat-state.md`。 |
