...
| 51 | Review 模块：双端点架构 | 复习模块不引入 WebSocket，使用两个 REST 端点完成完整复习循环：① `GET /api/review/next?deckId=X&mode=Y`（获取首张卡片，按 Mode 策略选卡）；② `POST /api/review/rate {cardId, rating, deckId, mode}`（评分当前卡 → 更新 FSRS 状态 → computeStats → getNextCard，三合一返回 `{card, nextCard, stats}`）。前端在收到 `nextCard` 后原地替换当前卡片展示，不单独再次请求 next，也不在前端维护任何累积计数。此设计让 ReviewPage 只需管理当前卡片和翻面状态，无需跨请求状态同步。 |
...
| 53 | 每日新卡上限后端化 | `newCardDailyLimit` 存储在 `UserPreferences` 表，由后端 `ReviewService.getNextCard()` 和 `computeStats()` 统一读取。DeckPicker 点击"开始"时通过 `PUT /api/user/preferences` 将用户输入的上限值保存到数据库；后续 `GET /api/review/next` 和 `POST /api/review/rate` 请求不再携带 `limit` 参数——后端自行从 `prefs.getNewCardDailyLimit()` 读取。此前 `computeStats()` 将 `dailyLimit` 硬编码为 20 且使用错误的 `Instant.now()` 阈值（非 `todayStart`）导致 StatsBar 和结算页数字错误——本次一并修正。 |
