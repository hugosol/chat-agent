...
- **Frontend**: migrating to React + TypeScript in `src/main/frontend/` (Vite build → `static/`). Currently: Header + CorrectionSidebar React modules + shared utility layer (`src/shared/`: `utils.ts`, `tts.ts`, `Modal.tsx`, `Toast.tsx`, `ChipInput.tsx`, `useTagAutocomplete.ts`, `types.ts`) use React; chat (`app.js`), flashcard (`flashcard.js`), manage panels (`manage/*.js`) remain vanilla JS. Tests organized by functional domain under `src/__tests__/` (`shared/`, `header/`, `correction-sidebar/`). `package.json` at `src/main/frontend/`. Multi-entry build: two Vite configs (`vite.config.ts` + `vite.config.correction-sidebar.ts`) produce `header-bundle.js`/`.css` and `correction-sidebar-bundle.js`/`.css`. Vite build triggered by `exec-maven-plugin` during `process-resources`; Vitest (`npm test`) triggered during `process-test-classes`. React hosted locally in `static/shared/` (no CDN dependency). Vite Library Mode extracts CSS to separate file: each HTML page must include both `<link>` to CSS bundle and `<script>` to JS bundle. Shared code is inlined into each component bundle (no separate shared bundle). CorrectionSidebar uses **pure component + connector pattern**: `CorrectionSidebar.tsx` is a pure display component (props in, callbacks out), `correction-sidebar-entry.tsx` manages state via `useState` and exposes imperative API (`addCorrection`/`clear`/`getCount`) to vanilla JS. This pattern forward-compatible with planned `useReducer + context` centralized state management (Phase 3). Gotchas: (1) `define` `process.env.NODE_ENV` only for build, not test — or Vitest breaks; (2) root element inside flex container needs `flex:1;min-width:0`; (3) use `build.lib.cssFileName` for stable CSS filename; (4) IIFE format doesn't support multi-entry in `build.lib.entry` — use separate Vite configs; (5) CSS Modules hashes class names, E2E tests must use `data-testid` attributes; (6) each HTML page must include `<link>` to every CSS bundle it uses. Full details in `docs/adr/frontend-react-migration.md#implementation-notes`. Correction sidebar rendered by React: collapsed state via `aria-expanded` (CSS `display: none` when `aria-expanded="false"`); floating ⚠️ N ◂ badge conditionally rendered when corrections exist. Flashcard panel (`flashcard.js`) independent of `app.js`, shared via `window.activePanel`. Login page at `/login/main.html` with dark theme, served from `static/login/`. Manage page at `/manage/index.html` — card/tag CRUD, search, sort, deck chip filtering, pagination, TTS.
