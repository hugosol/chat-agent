...
- **Frontend**: migrating to React + TypeScript in `src/main/frontend/` (Vite build → `static/`). Currently: shared Header component + shared utility layer (`src/shared/`: `utils.ts`, `tts.ts`, `Modal.tsx`, `Toast.tsx`, `ChipInput.tsx`, `useTagAutocomplete.ts`) use React; chat (`app.js`), flashcard (`flashcard.js`), manage panels (`manage/*.js`) remain vanilla JS. Tests organized by functional domain under `src/__tests__/` (`shared/`, `header/`). `package.json` at `src/main/frontend/`. Vite build triggered by `exec-maven-plugin` during `process-resources`; Vitest (`npm test`) triggered during `process-test-classes`. React hosted locally in `static/shared/` (no CDN dependency). Global namespace: `window.ChatAgent.mountHeader()`. Vite Library Mode extracts CSS to separate file: each HTML page must include both `<link>` to CSS bundle and `<script>` to JS bundle. Shared code is inlined into each component bundle (no separate shared bundle). Gotchas: (1) `define` `process.env.NODE_ENV` only for build, not test — or Vitest breaks; (2) root element inside flex container needs `flex:1;min-width:0`; (3) use `build.lib.cssFileName` for stable CSS filename. Full details in `docs/adr/frontend-react-migration.md#implementation-notes`. Correction sidebar hidden when collapsed (`display: none`); floating ⚠️ N ◂ badge appears at center-right when corrections arrive. Click badge to expand 260px sidebar, click ▸ in header to collapse. Flashcard panel (`flashcard.js`) independent of `app.js`, shared via `window.activePanel`. Login page at `/login/main.html` with dark theme, served from `static/login/`. Manage page at `/manage/index.html` — card/tag CRUD, search, sort, deck chip filtering, pagination, TTS.
