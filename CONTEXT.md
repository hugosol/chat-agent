...
| **MemoryAssertion** | A structured memory record in the `memory_assertions` table — one row per independent fact extracted from a conversation segment. Each assertion has a `topic` (short concept label, e.g. "past tense"), a `state` (one-sentence natural language description), and an `enabled` flag (false when superseded by a merge). Grouped by AssertionGroup. Runs in parallel with MemoryCue in the write pipeline for non-Japanese modes. V1: retrieval still uses MemoryCue. | Assertion, structured memory, fact record |
| **AssertionGroup** | A classification dimension for assertions, stored in `assertion_groups`. Each group defines what counts as "a concept" in its dimension via a `description` field used as an LLM prompt parameter, and a `mode` column binding it to a specific `AgentMode`. V1 has two groups: `error-pattern` (grammar/word choice error patterns) and `dev-progress` (daily development progress, blockers, technologies). Both bound to `WORKPLACE_STANDUP` mode. Design is extensible — adding a group is an INSERT with `mode` set, no code change. `generateAssertionsAsync()` queries `findByMode(mode)` and processes groups serially; modes without groups (e.g. `DAILY_TALK`) silently skip the assertion pipeline. | Group, assertion category |
| **AssertionLineage** | An edge table (`assertion_lineage`) recording how assertions evolve. Each row links a parent assertion to its child with an `operation` type (default MERGE). Supports recursive CTE queries to trace an assertion's full evolutionary DAG — e.g., assertion-5 was merged from assertion-3 and assertion-4, while assertion-3 was merged from assertion-1 and assertion-2. | Lineage, evolution chain, ancestry |
| **Extractor** | Phase 1 of the assertion pipeline. Uses a two-step LLM process per conversation segment × AssertionGroup: Step1 extracts topic concepts (e.g., "past tense irregular verbs") using a parameterized prompt with `{groupName}` and `{groupDescription}`; Step2 generates a one-sentence `state` for each topic using a constrained fill-in-the-blank prompt. Topics from Step1 act as statistical anchors — narrowing the LLM's task from open-ended summarization to answering "how is the user doing with this specific topic?" | Topic extraction, state generation |
| **Manager** | Phase 2 of the assertion pipeline. Processes newly extracted assertions sequentially to avoid race conditions. For each new assertion: (1) embedding search against the independent assertion embedding store returns top-3 semantically similar old assertions; (2) per-candidate Judge LLM call returns YES/NO for semantic equivalence; (3) for each YES, a Merge LLM call combines both assertions' states into one, the old assertion is soft-deleted (`enabled=false`) and removed from the embedding index, the merged result is inserted as a new assertion, and an AssertionLineage edge is recorded. | Merge pipeline, assertion dedup |
| **Assertion Embedding Store** | An independent `InMemoryEmbeddingStore` separate from the MemoryCue embedding store. Stores vectors of `MemoryAssertion.state` text (not topic) for Manager search(top-3). `enabled=false` assertions are removed from this store. Persisted to disk as a separate JSON file. | Assertion vector store, assertion index |

## Observability

| Term | Definition | Aliases to avoid |
|------|------------|----------------|
| **LLM Call Log** | Every LLM API call is persisted as a row in the `llm_call_logs` table: `request_prompt` (full prompt blob), `system_prompt` and `chat_history` (split for structured querying), `response_text`, token usage (input/output), duration (ms), and status (SUCCESS/ERROR). Sync agent calls are logged by `LlmReqConstructor` with full `systemPrompt`/`chatHistory`/`inputTokens`/`outputTokens` fields; the ConversationAgent (streaming) is logged manually via `TurnProcessor`. Writes are async (non-blocking) and records older than 3 days are cleaned up on startup. Used for debugging and cost tracking. | LLM log, call log, API log |
...
|| **Card uniqueness** | A validation rule: within the same User, a `(front, tag)` pair must be unique (case-insensitive on front). Same-Deck duplicates are hard-blocked (422); cross-Deck duplicates are allowed (independent cards with separate FSRS scheduling). The `POST /api/cards/check` endpoint returns conflicting tag information for frontend use. | 卡片防重 |
...
| **Requote** | The Replace button (🔄) in the Movie Quote Zone that triggers `POST /api/cards/{id}/enhance/requote`. Excludes the currently displayed `(imdbId, timestamp)` pair from candidates, randomly selects a different matching subtitle line from the Learner's movie library, regenerates the scene summary via LLM, and persists the new data to `card_enhancements`. When no current quote exists (placeholder state), performs a fresh full search. Returns `{ found: false }` when no other matching quote is available. | Replace, swap quote, refresh quote |
| **Movie Quote Zone** | The UI section on a flashcard back showing a matched movie quote: movie title header row (title + timestamp on left, TTS button 🔊 + Requote button 🔄 on right), quote text in italics, and optional scene summary. Rendered alongside etymology when enhancement data is available. The TTS button reads the quote aloud via browser SpeechSynthesis; the Requote button opens a confirmation dialog and replaces the current quote with a different one from the database. | Quote zone, movie quote display |
...
- A **Practice session** end triggers **Assertion Extraction**: **Extractor** (two-step LLM per segment×group → MemoryAssertion rows with `enabled=true` + embedding index) followed by **Manager** (sequential search→judge→merge per new assertion, producing lineage edges and soft-deleting superseded assertions). Runs in parallel with MemoryCue generation in the write pipeline for non-Japanese modes. Japanese mode is skipped.
- An **AssertionGroup** has zero or more **MemoryAssertions**.
- A **MemoryAssertion** may be the parent of zero or more child assertions via **AssertionLineage** edges, forming a directed acyclic graph of memory evolution.
...
- A **Requote** replaces an existing **Card Enhancement** subtitle entry with a different match from the same Learner's movie library, excluding the currently displayed line. It triggers a fresh LLM call for scene summary generation.
...
- **State** — with the introduction of MemoryAssertion, "state" now has three meanings: (1) ChatState (the langgraph container), (2) the Active states map (collection of ChatState instances), and (3) MemoryAssertion.state (the one-sentence natural language assertion text). **Resolution:** Use "ChatState" for the langgraph container, "Active states map" for the collection, and "assertion state" or "state field" for MemoryAssertion's text content. Never say "state" alone.
...
>
> **Dev:** "What happens when the Learner makes the same grammar mistake across two sessions?"
>
> **Domain expert:** "At the end of each session, the Extractor pulls out topics like 'past tense irregular verbs' and generates an assertion state. On session 2, the Manager finds the session 1 assertion via embedding search, Judge confirms they're about the same thing, and Merge combines them into a new assertion that says 'the user has been struggling with irregular past tense across two sessions'. The old assertions get enabled=false and lineage edges connect them to the merged result."
>
> **Dev:** "So I can trace back that a merged assertion came from two earlier ones?"
>
> **Domain expert:** "Exactly. The assertion_lineage table stores parent→child edges. A recursive CTE from any assertion id gives you its full evolutionary tree. If assertion-5 was merged from 3 and 4, and 3 was merged from 1 and 2, one query traces the whole chain."
