...
| **AssertionGroup** | A classification dimension for assertions, stored in `assertion_groups`. Each group defines what counts as "a concept" in its dimension via a `description` field used as an LLM prompt parameter. V1 has a single group: `error-pattern` ("user在对话中出现的语法/用词错误类型"). Future groups include `learning-goal`, `personal-fact`. Design is extensible — adding a group is an INSERT, not a code change. | Group, assertion category |
